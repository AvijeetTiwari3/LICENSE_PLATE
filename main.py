...
def recognize_plate(plate_crop):
    ...
    try:
        ocr_result = reader.readtext(plate_resized , details = 0 , allowlist = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789")
        if len(ocr_result)>0:
...
# out = cv2.VideoWriter(output_video , fourcc , 20.0 , cap.get(cv2.CAP_PROP_FPS) ,(int(cap.get(3)) ,  int(cap.get(4))))
            ...
            if plate_crop.size >0 :
                ...
                if oy2<= frame.shape[0] and ox2 <= frame.shape[1]:
                    ...
                    if stable_text:
                        ...
                        cv2.putText(frame , stable_text , (ox1 , oy1 -20) , cv2.FONT_HERSHEY_SIMPLEX , 2 , (255 ,255 ,255) ,2)
...
print("Video processing complete! Output saved at:", output_video)
